...
const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, formUrl }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white dark:bg-gray-700 rounded-lg p-6 w-96">
        ...
        <h2 className="text-lg font-bold dark:text-gray-200 text-gray-700">Share Your Form</h2>
        ...
        <div className="mt-4 flex items-center gap-2">
          <input
            ...
            onFocus={handleCopy}
          />
          <button onClick={handleCopy} className=" text-sm dark:text-white text-gray-600">
        ...
        <div className="mt-4">
          <input
            ...
            required
          />
          {error && error.length > 0 && <p className='text-sm text-red-500'>{error}</p>}
          <div className="flex justify-end w-full">
          <Button onClick={() => handleSendEmail(formUrl,email)} className="mt-2">
            Send Email
          </Button>
          </div>
        </div>
